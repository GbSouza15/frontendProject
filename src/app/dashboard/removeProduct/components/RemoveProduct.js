...
export default function RemoveProduct() {
    ...
    const handleDeleteProduct = (productId) => {
        fetch(`${url}/product/remove/${productId}`, {
            method: 'DELETE',
            headers: {
            ...
            if (response.ok) {
              // Produto excluído com sucesso, atualize a lista de produtos
              setProducts(products.filter(product => product.id !== productId));
              alert('Produto excluído com sucesso!')
            } else {
              // Tratar erros ou exibir uma mensagem de erro ao usuário
              alert('Ocorreu um erro ao excluir o produto.');
            }
          })
          .catch(error => {
            // Tratar erros de conexão ou exibir uma mensagem de erro ao usuário
    ...
    return (
        <div className="mt-[5rem] mb-[15rem]">
            <h1 className="text-center font-bold text-[1.5rem]">Remover Produtos</h1>
                ...
                {products.map(product => (
                    <div key={product.id} className="bg-black w-[20rem] h-[18rem] mt-[1.5rem] rounded-[.2rem]">
                        ...
                        <div className="mt-[2rem] text-white pl-[.5rem]">
                            <p>{product.name}</p>
                            <button onClick={() => handleDeleteProduct(product.id)} className="bg-red-700 p-1 rounded-[.5rem] mt-[.5rem]">DELETAR</button>
                        </div>
                    </div>
                ))}
            </div>
